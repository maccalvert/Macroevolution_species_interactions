...
# ******* Note - Due to occasional issues with NCBI servers, the underlying "curl" calls (which are used to access NCBI)
# may timeout. If this occurs simply run the function again. If timeouts happen regularly, your query may be returing 
#too many records or there may be a connection issue on your end. You can 
# edit the curl defaults using the httr package - see options using httr_options() such that curl spends more time trying to 
# make a conneection. 
    ...
    if(object.size(rec) > 100000){
      ...
      if(is.na(nat_host) & is.na(isolation_source)){
        ...
        if(sum(match_ind) > 0){
          ...
          split_source <- gsub("[[:punct:]]", "", split_source)
